...
{
    public partial class HardTest2 : Form
    {
        ...
        public HardTest2(string u)
        {
            ...
            int random_sequence = rnd.Next(1, 5);  // Random number of true false questions
            int random_fillGap = 5 - random_sequence; // The remaining questions are of type multiple choice

            // Retrieve the multiple choice questions from database
            string query = "select * from fillgap inner join questions on fillgap.id= questions.id where chapter=2";
            List<FillGap> fillGapList = new List<FillGap>(); // List containing all of the multiple choice questions
            try
            {
            ...
            string query2 = "select * from sequence inner join questions on sequence.id= questions.id where chapter=2";
            List<Sequence> sequenceList = new List<Sequence>(); // List containing all of the true false questions
            try
            {
                NpgsqlConnection connection = new NpgsqlConnection(Auxiliary.CONNECTION_STRING);
                connection.Open();
        ...
        private void HardTest2_MouseDown(object sender, MouseEventArgs e)
        {
            _dragging = true;
            _start_point = new Point(e.X, e.Y);
        }

        private void HardTest2_MouseMove(object sender, MouseEventArgs e)
        {
            if (_dragging)
            {
        ...
        private void HardTest2_MouseUp(object sender, MouseEventArgs e)
        {
            _dragging = false;
        }

        // Key shortcuts
        private void HardTest2_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            else
            {
                ...
                if (i == 5) // User answered all the questions go back to main menu
                {
                    ...
                    try
                    {
                        ...
                        command.Parameters.AddWithValue("chapter", 2);
