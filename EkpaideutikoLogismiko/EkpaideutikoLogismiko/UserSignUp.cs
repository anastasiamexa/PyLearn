...
namespace EkpaideutikoLogismiko
{
    public partial class UserSignUp : Form
    {
        ...
        private void UserSignUp_MouseDown(object sender, MouseEventArgs e)
        {
            _dragging = true;
            _start_point = new Point(e.X, e.Y);
        }

        private void UserSignUp_MouseMove(object sender, MouseEventArgs e)
        {
            if (_dragging)
            {
        ...
        private void UserSignUp_MouseUp(object sender, MouseEventArgs e)
        {
            _dragging = false;
        }
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        // Create user object
                        User user = new User(firstname.Text.ToString(), lastname.Text.ToString(), username.Text.ToString(), hashed_password, salt, false);
                        string s = user.StoreUserInfoToDB();
                        if (s == "done")
                        {
